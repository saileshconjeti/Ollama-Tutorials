...
if __name__ == "__main__":
    ...
    print("What this demonstrates: the agent extracts useful user facts, stores them, and sends selected memory into the reply prompt.")
    ...
    print_step(3, "Loading memory from disk")
    if memory_exists:
    ...
    print_step(4, "Extracting memory updates from message")
    ...
    print_step(5, "Building reply with memory context")
    reply = build_reply(
        user_message,
        memory,
        model=selected_model,
    ...
    print("The stored JSON file is the long-term memory; the prompt decides how much of it influences the current answer.")
